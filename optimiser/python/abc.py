...
def abc(func, nvar, lower_bound=None, upper_bound=None, setting=None):
    ...
    """
    if nvar <= 0:
        raise ValueError("Dimension cannot be zero or negative")
    ...
    if lower_bound.shape != upper_bound.shape:
        raise ValueError("Dimensions of bounds mismatch")
    if lower_bound.ndim != 1 or upper_bound.ndim != 1:
        raise ValueError("Bounds must be a vector")
...
def display(setting, iteration, fval):
    ...
    if display_setting == "iter":
        print(f"Generation: {iteration}\t f(x): {fval}")
...
def abcoptions(*varargin):
    ...
    if len(varargin) == 0:
        print("initial_population  : [matrix]")
        print('Generations        : [positive scalar | "200"]')
        print("Display            : ['off' | 'iter' | 'final']")
        print('PopulationSize     : [positive scalar | "200"]')
        print('Limit              : [positive scalar | "10"]')
        print('StallLimit         : [positive scalar | "Inf"]')
        return None
