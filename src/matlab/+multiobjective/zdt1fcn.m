...
% SCORES = ZDT1FCN(X) computes the value of the ZDT1 function
% at point X. ZDT1FCN accepts a matrix of size M-by-N and returns
...
function scores = zdt1fcn(x)
    n = size(x, 2);
    assert(n >= 2, 'The ZDT1 function requires at least a 2D space.');
    m = size(x, 1);
    scores = zeros(m, 2);
    scores(:, 1) = x(:, 1);
