...
namespace BenchmarkFcns {
    ...
    VectorXd ackley(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        return apply_parallel(x, [](const auto& a) {
            ...
            return VectorXd(-(20.0 * (-0.2 * (sum1.array() / n).sqrt()).exp()) + 20.0 - (sum2.array() / n).exp() + exp(1.0));
        });
    }

    VectorXd ackleyn2(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("Ackley2 function only accepts 2D input.");
        return apply_parallel(x, [](const auto& a) {
    ...
    VectorXd ackleyn3(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Ackleyn3 function only accepts 2D inputs.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(-200.0 * ((-0.2 * (X.square() + Y.square()).sqrt()).exp()) + 5.0 * ((3.0 * X).cos() + (3.0 * Y).sin()).exp());
        });
    }
    ...
    VectorXd ackleyn4(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        return apply_parallel(x, [](const auto& a) {
            const int n = a.cols();
            const MatrixXd term1 = exp(-0.2) * ((a.block(0, 0, a.rows(), n - 1).square() + a.block(0, 1, a.rows(), n - 1).square()).sqrt());
            const MatrixXd term2 = 3.0 * ((2.0 * a.block(0, 0, a.rows(), n - 1)).cos() + (2.0 * a.block(0, 1, a.rows(), n - 1)).sin());
            return VectorXd((term1 + term2).rowwise().sum());
        });
    }

    VectorXd adjiman(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Adjiman function only accepts 2D inputs.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd alpinen2(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        return apply_parallel(x, [](const auto& a) {
            return VectorXd((a.sqrt() * a.sin()).rowwise().prod());
        });
    }
    ...
    VectorXd bartelsconn(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Bartels Conn function only accepts 2D inputs.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd beale(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Beale function only accepts 2D inputs.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd((1.5 - X + (X * Y)).square() +
                            (2.25 - X + (X * Y.square())).square() +
    ...
    VectorXd biggsexp02(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The BiggsExp02 function only accepts 2D inputs.");
        ...
        return apply_parallel(x, [ti, yi, numTerms](const auto& a) {
            ...
            const MatrixXd residuals_sq = (term1.array() - term2.array() - yi.replicate(1, chunk_rows).array()).square();
            return VectorXd(residuals_sq.colwise().sum());
        });
    }

    VectorXd biggsexp03(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The BiggsExp03 function only accepts 2D inputs.");
        const int numTerms = 10;
        const VectorXd i = VectorXd::LinSpaced(numTerms, 1, numTerms);
        const VectorXd ti = 0.1 * i;
        const VectorXd yi = (-ti).array().exp() - 5 * (-10 * ti).array().exp();
    ...
    VectorXd biggsexp04(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 3) throw std::invalid_argument("The BiggsExp04 function only accepts 3D inputs.");
        const int numTerms = 10;
        const VectorXd i = VectorXd::LinSpaced(numTerms, 1, numTerms);
        const VectorXd ti = 0.1 * i;
        const VectorXd yi = (-ti).array().exp() - 5 * (-10 * ti).array().exp();
    ...
    VectorXd biggsexp05(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 4) throw std::invalid_argument("The BiggsExp05 function only accepts 4D inputs.");
    ...
    VectorXd biggsexp06(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 6) throw std::invalid_argument("The BiggsExp06 function only accepts 6D inputs.");
        ...
        return apply_parallel(x, [ti, yi, numTerms](const auto& a) {
            ...
            const MatrixXd term1 = x3.replicate(numTerms, 1).array() * (-(ti.replicate(1, chunk_rows).array() + x5.replicate(numTerms, 1).array()) * x1.replicate(numTerms, 1).array()).exp();
            const MatrixXd term2 = x4.replicate(numTerms, 1).array() * (-(ti.replicate(1, chunk_rows).array() + x6.replicate(numTerms, 1).array()) * x2.replicate(numTerms, 1).array()).exp();
    ...
    VectorXd bird(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Bird function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd((X.sin() * (1 - Y.cos()).square().exp()) + (Y.cos() * (1 - X.sin()).square().exp()) + (X - Y).square());
        });
    }

    VectorXd bohachevskyn1(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Bohachevsky N. 1 function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(X.square() + 2 * Y.square() - 0.3 * (3 * M_PI * X).cos() - 0.4 * (4 * M_PI * Y).cos() + 0.7);
        });
    }

    VectorXd bohachevskyn2(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Bohachevsky N2 function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(X.square() + 2 * Y.square() - 0.3 * (3 * M_PI * X).cos() * (4 * M_PI * Y).cos() + 0.3);
        });
    }

    VectorXd bohachevskyn3(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Bohachevsky N3 function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(X.square() + 2 * Y.square() - 0.3 * (3 * M_PI * X + 4 * M_PI * Y).cos() + 0.3);
        });
    }

    VectorXd booth(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Booth function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd braninn1(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Branin N. 1 function only accepts 2D inputs");
        ...
        return apply_parallel(x, [a_coeff, b, c, d, e, f](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(a_coeff * (Y - b * X.square() + c * X - d).square() + e * (1 - f) * X.cos() + e);
        });
    }

    VectorXd braninn2(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Branin N. 2 function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd((X.square() + Y.square() - 5).square() + (X.sin().square() + Y.cos().square() - 1).square());
        });
    }

    VectorXd brent(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Brent function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd bukinn2(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Bukin N. 2 function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd bukinn4(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Bukin N. 4 function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd bukinn6(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Bukin N. 6 function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd carromtable(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Carrom Table function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(-((X.cos() * Y.cos() * (1 - (X.square() + Y.square()).sqrt() / M_PI).abs().exp()).square() / 30.0));
        });
    }

    VectorXd chichinadze(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Chichinadze function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(X.square() - 12 * X + 11 + 10 * (M_PI * X / 2.0).cos() + 8 * (5 * M_PI * X / 2.0).sin() - (1.0 / sqrt(2.0 * M_PI)) * (-0.5 * (Y - 0.5).square()).exp());
        });
    }
    ...
    VectorXd corana(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        VectorXd d(n);
        for (int i = 0; i < n; ++i) d(i) = d_base(i % 4);
    ...
    VectorXd crossintray(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Cross-in-tray function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            const VectorXd expcomponent = (100 - ((X.square() + Y.square()).sqrt() / M_PI)).abs();
            return VectorXd(-0.0001 * ((X.sin() * Y.sin() * expcomponent.array().exp()).abs() + 1).pow(0.1));
        });
    }

    VectorXd crownedcross(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Crowned Cross function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            const VectorXd expcomponent = (100 - ((X.square() + Y.square()).sqrt() / M_PI)).abs();
    ...
    VectorXd cubefcn(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Cube function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd debn1(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        return apply_parallel(x, [](const auto& a) {
            return VectorXd(-( (5 * M_PI * a).sin().pow(6).rowwise().sum() ));
        });
    }

    VectorXd deckkersaarts(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Deckkers-Aarts function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd dejongn5(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("De Jong's N. 5 function is only defined on a 2D space.");
    ...
    VectorXd dropwave(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Drop Wave function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
    ...
    VectorXd easom(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Easom function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd eggcrate(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Egg Crate function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd eggholder(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Eggholder function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(-(Y + 47) * ( (X / 2.0 + Y + 47).abs().sqrt() ).sin() - X * ( (X - (Y + 47)).abs().sqrt() ).sin());
        });
    }

    VectorXd elattar(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The El-Attar et al. function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd((X.square() + Y - 10).square() + (X + Y.square() - 7).square() + (X.square() + Y.cube() - 1).square());
        });
    }
    ...
    VectorXd elliptic(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        const int n = x.cols();
        VectorXd powers(n);
        for(int i=0; i<n; ++i) powers(i) = std::pow(10.0, 6.0 * i / (n - 1.0));
    ...
    VectorXd forrester(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 1) throw std::invalid_argument("The Forrester function is only defined on a 1D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
    ...
    VectorXd foxholes(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Foxholes function only accepts 2D inputs");
        static constexpr double a_content[2][25] = {
            {-32, -16, 0, 16, 32, -32, -16, 0, 16, 32, -32, -16, 0, 16, 32, -32, -16, 0, 16, 32, -32, -16, 0, 16, 32},
            {-32, -32, -32, -32, -32, -16, -16, -16, -16, -16, 0, 0, 0, 0, 0, 16, 16, 16, 16, 16, 32, 32, 32, 32, 32}
        };
        const Matrix<double, 2, 25> a_mat = Map<const Matrix<double, 2, 25, RowMajor>>(&a_content[0][0]);
    ...
    VectorXd friedman1(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x, bool rnd) {
        if (x.cols() != 10) throw std::invalid_argument("The Friedman No. 1 function is only defined on a 10D space.");
        return apply_parallel(x, [rnd](const auto& a) {
            ...
            if (rnd) epsilon = VectorXd::Random(a.rows());
    ...
    VectorXd friedman2(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x, double sigma) {
        if (x.cols() != 4) throw std::invalid_argument("The Friedman No. 2 function is only defined on a 4D space.");
        return apply_parallel(x, [sigma](const auto& a) {
            ...
            VectorXd epsilon = VectorXd::Zero(a.rows());
            if (sigma > 0) epsilon = sigma * VectorXd::Random(a.rows());
    ...
    VectorXd friedman3(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x, double sigma) {
        if (x.cols() != 4) throw std::invalid_argument("The Friedman No. 3 function is only defined on a 4D space.");
        return apply_parallel(x, [sigma](const auto& a) {
            const auto x1 = a.col(0);
            const auto x2 = a.col(1);
            const auto x3 = a.col(2);
    ...
    VectorXd gallagher101(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        if (!initialized) {
            ...
            for (int i = 1; i < 101; ++i) {
                ...
                for (int j = 0; j < n; ++j) y_vec[i](j) = dist_x(gen);
        ...
        return apply_parallel(x, [n](const auto& a) {
            ...
            for (int i = 0; i < 101; ++i) {
                ...
                VectorXd exponent = -0.5 / static_cast<double>(n) * (diff * C_vec[i]).array().square().rowwise().sum();
    ...
    VectorXd gear(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 4) throw std::invalid_argument("The Gear function only accepts 4D inputs");
        return apply_parallel(x, [](const auto& a) {
    ...
    VectorXd giunta(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        return apply_parallel(x, [](const auto& a) {
            return VectorXd(0.6 + ( (1 - 16.0 / 15.0 * a).sin() + (1 - 16.0 / 15.0 * a).sin().square() + 1.0 / 50.0 * (4 * 16.0 / 15.0 * a).sin() ).rowwise().sum());
        });
    }

    VectorXd goldsteinprice(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Goldstein-Price function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd gramacylee(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 1) throw std::invalid_argument("The Gramacy & Lee function only accepts 1D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
    ...
    VectorXd happycat(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x, double alpha) {
        ...
        return apply_parallel(x, [n, alpha](const auto& a) {
            ...
            return VectorXd((x2.array() - n).square().pow(alpha) + (0.5 * x2.array() + sx.array()) / n + 0.5);
        });
    }

    VectorXd hartmann3(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 3) throw std::invalid_argument("The Hartmann 3 function only accepts 3D inputs");
        const VectorXd alpha = (VectorXd(4) << 1.0, 1.2, 3.0, 3.2).finished();
        const MatrixXd A = (MatrixXd(4, 3) << 3, 10, 30, 0.1, 10, 35, 3, 10, 30, 0.1, 10, 35).finished();
        const MatrixXd P = 1e-4 * (MatrixXd(4, 3) << 3689, 1170, 2673, 4699, 4387, 7470, 1091, 8732, 5547, 381, 5743, 8828).finished();
        ...
        return apply_parallel(x, [alpha, A, P](const auto& a) {
            ...
            for (int i = 0; i < 4; i++) {
                auto rowA = A.row(i);
                auto rowP = P.row(i);
                const VectorXd arg = (rowA.replicate(chunk_rows, 1).array() * (a.matrix().rowwise() - rowP).array().square()).rowwise().sum();
    ...
    VectorXd hartmann6(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 6) throw std::invalid_argument("The Hartmann 6 function only accepts 6D inputs");
        const VectorXd alpha = (VectorXd(4) << 1.0, 1.2, 3.0, 3.2).finished();
        const MatrixXd A = (MatrixXd(4, 6) << 10, 3, 17, 3.5, 1.7, 8,
                                             0.05, 10, 17, 0.1, 8, 14,
                                             3, 3.5, 1.7, 10, 17, 8,
                                             17, 8, 0.05, 10, 0.1, 14).finished();
        const MatrixXd P = 1e-4 * (MatrixXd(4, 6) << 1312, 1696, 5569, 124, 8283, 5886,
    ...
    VectorXd ishigami(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x, double a_param, double b_param) {
        if (x.cols() != 3) throw std::invalid_argument("The Ishigami function is only defined on a 3D space.");
    ...
    VectorXd himmelblau(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Himmelblau function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd holdertable(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Holder Table function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(-( (X.sin() * Y.cos() * (1 - (X.square() + Y.square()).sqrt() / M_PI).abs().exp()).abs() ));
        });
    }

    VectorXd hosaki(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Hosaki function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd((1 - 8 * X + 7 * X.square() - 7.0 / 3.0 * X.cube() + 1.0 / 4.0 * X.pow(4)) * Y.square() * (-Y).exp());
        });
    }
    ...
    VectorXd keane(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Keane's function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(-( (X - Y).sin().square() * (X + Y).sin().square() ) / (X.square() + Y.square()).sqrt());
        });
    }

    VectorXd langermann(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Langermann function is only defined on a 2D space.");
        ...
        return apply_parallel(x, [](const auto& a) {
            ...
            for (int i = 0; i < 5; ++i) {
                ...
                scores.array() -= c_vals[i] * ((-1.0/M_PI * sum_sq.array()).exp() * (M_PI * sum_sq.array()).cos());
            }
            return scores;
        });
    }
    ...
    VectorXd leon(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Leon function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd(100 * (Y - X.cube()).square() + (1 - X).square());
    ...
    VectorXd levin13(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Levi's function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd((3 * M_PI * X).sin().square() + (X - 1).square() * (1 + (3 * M_PI * Y).sin().square()) + (Y - 1).square() * (1 + (2 * M_PI * Y).sin().square()));
        });
    }
    ...
    VectorXd matyas(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Matyas' function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd mishrabird(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("Mishra's Bird function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto x1 = a.col(0);
            const auto x2 = a.col(1);
            return VectorXd(x2.sin() * (1.0 - x1.cos()).square().exp() + x1.cos() * (1.0 - x2.sin()).square().exp() + (x1 - x2).square());
        });
    }

    VectorXd mccormick(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The McCormick function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd michalewicz(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x, double m_param) {
        const int n = x.cols();
        const RowVectorXd I_vec_full = RowVectorXd::LinSpaced(n, 1, n);
    ...
    VectorXd picheny(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Picheny function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd((X.square() + Y - 11).square() + (X + Y.square() - 7).square());
    ...
    VectorXd powellsingular(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 4) throw std::invalid_argument("The Powell Singular function is only defined on a 4D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto x1 = a.col(0);
            const auto x2 = a.col(1);
            const auto x3 = a.col(2);
            const auto x4 = a.col(3);
            return VectorXd((x1 + 10.0 * x2).square() + 5.0 * (x3 - x4).square() + (x2 - 2.0 * x3).pow(4) + 10.0 * (x1 - x4).pow(4));
        });
    }
    ...
    VectorXd ridge(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x, double d_param, double alpha_param) {
        const int n = x.cols();
    ...
    VectorXd rosenbrock(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        return apply_parallel(x, [n](const auto& a) {
            ...
            return VectorXd((100 * (xi_plus_1 - xi.square()).square() + (xi - 1).square()).rowwise().sum());
        });
    }
    ...
    VectorXd schafferf6(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Schaffer F6 function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X2 = a.col(0).square();
            const auto Y2 = a.col(1).square();
    ...
    VectorXd schafferf7(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        return apply_parallel(x, [n](const auto& a) {
            ...
            for (int i = 0; i < n - 1; ++i) {
                ...
                scores.array() += si.array().sqrt() * ((50.0 * si.array().pow(0.2)).sin().square() + 1.0);
            }
            return scores;
        });
    }
    ...
    VectorXd schaffern1(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Schaffer N. 1 function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X2 = a.col(0).square();
            const auto Y2 = a.col(1).square();
    ...
    VectorXd schaffern2(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Schaffer N. 2 function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X2 = a.col(0).square();
            const auto Y2 = a.col(1).square();
    ...
    VectorXd schaffern3(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Schaffer N. 3 function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X2 = a.col(0).square();
            const auto Y2 = a.col(1).square();
    ...
    VectorXd schaffern4(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Schaffer N. 4 function is only defined on a 2D space.");
        return apply_parallel(x, [](const auto& a) {
            const auto X2 = a.col(0).square();
            const auto Y2 = a.col(1).square();
    ...
    VectorXd schwefel223(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        return apply_parallel(x, [](const auto& a) {
            return VectorXd(a.pow(10).rowwise().sum());
        });
    }
    ...
    VectorXd schwefel12(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        return apply_parallel(x, [n](const auto& a) {
            ...
            for (int i = 1; i < n; ++i) cumsums.col(i) = cumsums.col(i - 1).array() + a.col(i);
    ...
    VectorXd shekel5(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        const MatrixXd a_mat = (MatrixXd(5, 4) << 4, 4, 4, 4, 1, 1, 1, 1, 8, 8, 8, 8, 6, 6, 6, 6, 3, 7, 3, 7).finished();
        const VectorXd c_vec = (VectorXd(5) << 0.1, 0.2, 0.2, 0.4, 0.4).finished();
        return apply_parallel(x, [a_mat, c_vec](const auto& a) {
            VectorXd scores = VectorXd::Zero(a.rows());
            for (int i = 0; i < 5; i++) {
                scores.array() -= 1.0 / ((a.matrix().rowwise() - a_mat.row(i)).array().square().rowwise().sum() + c_vec(i));
            }
            return scores;
        });
    }
    ...
    VectorXd shekel7(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        const MatrixXd a_mat = (MatrixXd(7, 4) << 4, 4, 4, 4, 1, 1, 1, 1, 8, 8, 8, 8, 6, 6, 6, 6, 3, 7, 3, 7, 2, 9, 2, 9, 5, 5, 3, 3).finished();
    ...
    VectorXd shekel10(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        const MatrixXd a_mat = (MatrixXd(10, 4) << 4, 4, 4, 4, 1, 1, 1, 1, 8, 8, 8, 8, 6, 6, 6, 6, 3, 7, 3, 7, 2, 9, 2, 9, 5, 5, 3, 3, 8, 1, 8, 1, 6, 2, 6, 2, 2, 5, 8, 2).finished();
        const VectorXd c_vec = (VectorXd(10) << 0.1, 0.2, 0.2, 0.4, 0.4, 0.6, 0.3, 0.7, 0.5, 0.5).finished();
        return apply_parallel(x, [a_mat, c_vec](const auto& a) {
            VectorXd scores = VectorXd::Zero(a.rows());
            for (int i = 0; i < 10; i++) {
                scores.array() -= 1.0 / ((a.matrix().rowwise() - a_mat.row(i)).array().square().rowwise().sum() + c_vec(i));
    ...
    VectorXd shubert(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        return apply_parallel(x, [n](const auto& a) {
            ...
            for (int j = 1; j <= 5; j++) col_sums.array() += j * (((j + 1.0) * a + j).cos());
    ...
    VectorXd shubertn3(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        return apply_parallel(x, [n](const auto& a) {
            MatrixXd col_sums = MatrixXd::Zero(a.rows(), n);
            for (int j = 1; j <= 5; j++) col_sums.array() += j * (((j + 1.0) * a + j).sin());
    ...
    VectorXd shubertn4(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        return apply_parallel(x, [n](const auto& a) {
            MatrixXd col_sums = MatrixXd::Zero(a.rows(), n);
            for (int j = 1; j <= 5; j++) col_sums.array() += (((j + 1.0) * a + j).cos());
            return VectorXd(col_sums.rowwise().sum());
        });
    }
    ...
    VectorXd step(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        return apply_parallel(x, [](const auto& a) {
            return VectorXd((a + 0.5).floor().square().rowwise().sum());
        });
    }
    ...
    VectorXd threehumpcamel(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Three-hump Camel function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd sixhumpcamel(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Six-hump Camel function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd((4 - 2.1 * X.square() + X.pow(4) / 3.0) * X.square() + X * Y + (-4 + 4 * Y.square()) * Y.square());
        });
    }

    VectorXd treccani(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Treccani function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
    ...
    VectorXd trid(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        return apply_parallel(x, [n](const auto& a) {
            ...
            if (n > 1) scores -= (a.block(0, 1, m, n - 1) * a.block(0, 0, m, n - 1)).rowwise().sum().matrix();
            return scores;
        });
    }
    ...
    VectorXd vincent(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        return apply_parallel(x, [](const auto& a) {
            return VectorXd(-( (10 * a.sqrt()).sin() ).rowwise().sum());
        });
    }
    ...
    VectorXd watson(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        return apply_parallel(x, [n](const auto& a) {
            ...
            for (int i = 0; i < 31; i++) {
                ...
                for (int j = 1; j < n; j++) sum1.array() += j * a.col(j) * pow(ti, j - 1);
                VectorXd sum2 = VectorXd::Zero(m);
                for (int j = 0; j < n; j++) sum2.array() += a.col(j) * pow(ti, j);
    ...
    VectorXd wavy(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x, double k) {
        ...
        return apply_parallel(x, [n, k](const auto& a) {
            return VectorXd(1 - ( (k * a).cos() * (-a.square() / 2.0).exp() ).rowwise().sum() / static_cast<double>(n));
        });
    }

    VectorXd wayburnseadern2(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        if (x.cols() != 2) throw std::invalid_argument("The Wayburn-Seader N. 02 function only accepts 2D inputs");
        return apply_parallel(x, [](const auto& a) {
            const auto X = a.col(0);
            const auto Y = a.col(1);
            return VectorXd((1.613 - 4 * (X - 0.3125).square() - 4 * (Y - 1.625).square()).square() + (Y - 1).square());
        });
    }
    ...
    VectorXd weierstrass(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& X, double a_param, double b_param, int kmax) {
        const int n = X.cols();
    ...
    VectorXd xinsheyangn1(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        ...
        return apply_parallel(x, [exponents_full](const auto& a) {
            ...
            return VectorXd((rand_values.array() * a.abs().pow(exponents_full.replicate(rows, 1).array())).rowwise().sum());
        });
    }

    VectorXd xinsheyangn2(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x) {
        return apply_parallel(x, [](const auto& a) {
            return VectorXd(a.abs().rowwise().sum().array() * (-a.square().sin().rowwise().sum().array()).exp());
        });
    }

    VectorXd xinsheyangn3(const Ref<const Matrix<double,Dynamic,Dynamic,RowMajor>>& x, double beta, double m_param) {
